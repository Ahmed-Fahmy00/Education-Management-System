...
function StatusBadge({ status }) {
  return (
    <span
      className={`status-badge ${status === "available" ? "badge-available" : "badge-booked"}`}
    >
      {status === "available" ? (
        <>
...
function AvailableRoomsTab({ user }) {
  ...
  async function handleSearch(e) {
    ...
    setLoading(true);
    try {
      setRooms(await getAvailableRooms(filters));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    ...
    <div>
        ...
        <div className="filter-row">
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) => set("hasProjector", e.target.value)}
            >
      ...
      {rooms !== null && (
        <div className="results-section">
          ...
          {rooms.length === 0 ? (
            <p className="empty-msg">No rooms match your criteria.</p>
          ) : (
              ...
              {rooms.map((room) => (
                <div key={room._id} className="room-card">
                  ...
                  <div className="room-meta">
                    <span>
                      <Building2 size={13} /> {room.building}
                    </span>
...
function RoomStatusTab({ user }) {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setRooms(await getRoomsStatus(filters));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
        ...
        <div className="results-section">
          ...
          <div className="rooms-grid">
            {rooms.map((room) => (
              <div
                key={room._id}
                className={`room-card ${room.status === "booked" ? "room-card-booked" : ""}`}
              >
                <div className="room-card-header">
                  <span className="room-id-badge">#{room.roomId}</span>
                ...
                {room.status === "booked" && room.currentBooking && (
                  <div className="current-booking-info">
                    <p>
                      <strong>{room.currentBooking.title}</strong>
                    </p>
                    <p>
                      {room.currentBooking.bookedByName} ·{" "}
                      {fmtTime(room.currentBooking.startsAt)} –{" "}
                      {fmtTime(room.currentBooking.endsAt)}
                    </p>
                  </div>
                )}
...
function TimetableTab() {
  ...
  async function handleFetch(e) {
    ...
    setLoading(true);
    try {
      setTimetable(await getRoomTimetable(selectedRoomId, dates));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
        ...
        <div className="filter-row">
          <label>
            ...
            >
              ...
              {allRooms.map((r) => (
                <option key={r._id} value={r.roomId}>
                  #{r.roomId} — {r.name} ({r.building})
                </option>
              ))}
            </select>
          </label>
              ...
              <tbody>
                {timetable.bookings.map((b) => (
                  <tr key={b._id}>
                    ...
                    <td>
                      <Clock size={12} /> {fmtTime(b.startsAt)} –{" "}
                      {fmtTime(b.endsAt)}
                    </td>
                    <td>{b.title}</td>
                    <td>{b.bookedByName}</td>
                    <td>
                      <span className="role-tag">{b.bookedByRole}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function CalendarTab({ user }) {
  ...
  async function handleFetch(e) {
    ...
    setLoading(true);
    try {
      setEvents(await getCalendarBookings(filters));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
                ...
                {events.map((ev) => (
                  <tr key={ev.id}>
                    ...
                    <td>
                      ...
                      {ev.roomName}
                      <br />
                      <small className="muted">{ev.building}</small>
                    </td>
                    <td>{ev.title}</td>
                    <td>
                      {ev.bookedByName}
                      <br />
                      <span className="role-tag">{ev.bookedByRole}</span>
                    </td>
                    {canDelete && (
                      <td>
                        <button
...
export default function Rooms() {
  ...
  return (
    <UserLayout user={user} onLogout={handleLogout}>
      {/* Tab bar */}
      <div className="rooms-tab-bar">
        {TABS.map((tab, i) => (
          <button
            key={tab}
            className={`rooms-tab-btn ${activeTab === i ? "rooms-tab-active" : ""}`}
